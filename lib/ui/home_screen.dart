...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    // Read AppState synchronously (listen:false is allowed in initState)
    _appState = Provider.of<AppState>(context, listen: false);
    _tips = _safeGetTips(_appState);
    _startTipRotation();

    // Attach listener once to update tips only when they actually change.
  ...
  void _onAppStateChanged() {
    // Called whenever AppState notifies listeners. Update tips only if content changes.
    final newTips = _safeGetTips(_appState);
    if (!_listEquals(newTips, _tips)) {
      // Only update when tips content really changed (length or items)
      setState(() {
        _tips = newTips;
        _currentTipIndex = 0;
      });
  ...
  Widget _buildTodoItem(Reminder reminder, AppState appState) {
    ...
    for (var key in icons.keys) {
      if (reminder.title.toLowerCase().contains(key.toLowerCase())) {
        icon = icons[key]!;
        break;
      }
    }
        ...
        children: [
          ...
          GestureDetector(
            onTap: () async {
              ...
              // No local setState here; AppState will notify listeners and our AppState listener will only update tips when needed.
            },
